...
module "vpc" {
  ...
  transit_gateway        = local.use_transit_gateway ? module.tgw[0].id : ""
...
module "vpc-inspect" {
  ...
  flow_log_bucket        = module.s3-flow-log-bucket.bucket_arn
  transit_gateway        = module.tgw[0].id
...
resource "aws_route_table" "inspection_vpc_tgw_subnet_route_table" {
  count  = local.use_transit_gateway ? 2 : 0
  vpc_id = module.vpc-inspect[0].vpc_id
  route {
    cidr_block      = "0.0.0.0/0"
    vpc_endpoint_id = element([for ss in aws_networkfirewall_firewall.inspection_vpc_fw[0].firewall_status[0].sync_states : ss.attachment[0].endpoint_id if ss.attachment[0].subnet_id == module.vpc-inspect[0].firewall_subnets[count.index].id], 0)
  }
  tags = {
...
resource "aws_route_table_association" "inspection_vpc_tgw_subnet_route_table_association" {
  ...
  subnet_id      = module.vpc-inspect[0].tgw_subnets[count.index].id
}
...
resource "aws_route_table" "inspection_vpc_firewall_subnet_route_table" {
  ...
  route {
    ...
    transit_gateway_id = module.tgw[0].id
  }
  route {
    cidr_block     = "0.0.0.0/0"
    nat_gateway_id = module.vpc-inspect[0].nat_gateways[count.index].id
  }
  tags = {
...
resource "aws_route_table_association" "inspection_vpc_firewall_subnet_route_table_association" {
  ...
  subnet_id      = module.vpc-inspect[0].firewall_subnets[count.index].id
}
...
resource "aws_route_table" "inspection_vpc_public_subnet_route_table" {
  ...
  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = module.vpc-inspect[0].igw_id
  }
  route {
    cidr_block      = local.tgw_cidr_block
    vpc_endpoint_id = element([for ss in aws_networkfirewall_firewall.inspection_vpc_fw[0].firewall_status[0].sync_states : ss.attachment[0].endpoint_id if ss.attachment[0].subnet_id == module.vpc-inspect[0].firewall_subnets[count.index].id], 0)
...
resource "aws_route_table_association" "inspection_vpc_public_subnet_route_table_association" {
  ...
  subnet_id      = module.vpc-inspect[0].public_subnets[count.index].id
}


resource "aws_ec2_transit_gateway_vpc_attachment" "spoke" {
  for_each                                        = local.use_transit_gateway ? local.spoke_vpcs : {}
...
resource "aws_ec2_transit_gateway_vpc_attachment" "vpc-inspect" {
  count                                           = local.use_transit_gateway ? 1 : 0
  subnet_ids                                      = module.vpc-inspect[0].tgw_subnets.*.id
  transit_gateway_id                              = module.tgw[0].id
  vpc_id                                          = module.vpc-inspect[0].vpc_id
  transit_gateway_default_route_table_association = false
  transit_gateway_default_route_table_propagation = false
...
resource "aws_ec2_transit_gateway_route_table_association" "vpc_inspect_association" {
  count                          = local.use_transit_gateway ? 1 : 0
  transit_gateway_attachment_id  = aws_ec2_transit_gateway_vpc_attachment.vpc-inspect[0].id
  transit_gateway_route_table_id = module.tgw[0].inspection_route_table.id
}
...
resource "aws_ec2_transit_gateway_route" "blackhole" {
  for_each                       = local.use_transit_gateway ? local.blackhole_pairs : {}
